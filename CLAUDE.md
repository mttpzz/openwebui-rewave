...
- **Custom functions** live in `functions/` (Python) and are also DB rows — install/update them with `scripts/seed_functions.ps1` (idempotent: creates/refreshes code, sets `is_active` + `is_global` so they apply to `rewave-ai` without per-model assignment; OWUI loads the code live, no restart). Current: `functions/esporta_pdf.py`, a global **outlet filter** that auto-renders a `/presentazione` reveal.js reply to a branded landscape PDF via the **playwright** service (`ws://playwright:3000`; `playwright` is already in the openwebui image, no extra deps), stores it via the Files API, and **replaces the message** with just the title + a download link. So a redeploy needs only the seed scripts — no Admin UI clicks.
- **Model bootstrap** — `scripts/seed_model.ps1` creates the whole `rewave-ai` setup via API (idempotent create-or-update; same `.env` auto-load): the **RouteLLM connection** (`http://routellm:6060/v1`, auth=none, manual `model_ids: [router-bert-<threshold>]` since RouteLLM serves no `/v1/models`), the **`rewave-ai` model** (base = router, system prompt with empty wiki markers + the 3-source rules — `$systemPrompt` in the script is the **source of truth** for the prompt, params `temp 0.2`/`max_tokens 4096`, capabilities, Default Features all off incl. web_search-as-default, `esporta_pdf` action), **visibility** (`rewave-ai` + `router-bert-<threshold>` Public; `claude-sonnet`/`claude-haiku`/`gemma3-4b` Private — public = wildcard `user:*` read grant in the `access_grant` table, private = no grants), and the **Task Model = `claude-haiku`** (titles/tags/follow-ups). Visibility model in this OWUI version is the `access_grant` table (not `model.access_control`). Re-run with `-RouterModelId router-bert-<new>` after recalibration to repoint connection + base model together. The one irreducible manual step before any script: enabling the API Key feature + minting an admin key (chicken-and-egg).
- So a full from-scratch model setup is one script chain: `seed_model` → `refresh_wiki` → `seed_prompts` → `seed_suggestions` → `seed_functions`.
